/*
  ...
  useState: guarda se o usuário rolou a página (true/false)
  useEffect: "escuta" o evento de scroll do navegador
  
  Quando scrolled = true → adiciona classe "scrolled" ao header
  → isso muda o fundo de transparente para escuro (via CSS)
*/
...
function Header() {
  // Estado: a página foi rolada para baixo?
  const [scrolled, setScrolled] = useState(false);
  // Estado: o menu mobile está aberto?
  const [menuOpen, setMenuOpen] = useState(false);

  useEffect(() => {
    // Função chamada toda vez que o usuário rola a página
    const handleScroll = () => {
      // Se rolou mais de 80px → ativa o fundo escuro
  ...
  return (
    <header className={`header ${scrolled ? 'scrolled' : ''}`}>
      <div className="header__inner container">
        ...
        {/* LOGO — nome da empresa em duas linhas */}
        <Link to="/" className="header__logo">
          <span className="header__logo-nome">Gustavo</span>
          <span className="header__logo-sobrenome">Moreira</span>
        </Link>
        ...
        <nav className={`header__nav ${menuOpen ? 'open' : ''}`}>
          <ul className="header__nav-list">
            ...
            <li><Link to="/corporativo" onClick={() => setMenuOpen(false)}>Corporativo</Link></li>
        ...
        <button
          ...
          aria-label="Menu"
        >
          {/* Três linhas que viram X quando o menu abre */}
          <span></span>
          <span></span>
          <span></span>
        </button>
